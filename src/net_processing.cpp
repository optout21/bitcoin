                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        CBlockIndexOptRefConst pindex{*m_chainman.ActiveChainstate().FindForkInGlobalIndex(locator)};
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        CNodeState *nodestate = State(pfrom.GetId());
        CBlockIndexOptRefConst pindex{std::nullopt};
        if (locator.IsNull())
        {
            // If locator is null, return the hashStop block
            pindex = CBlockIndexOptRefConst(std::cref(*m_chainman.m_blockman.LookupBlockIndex(hashStop)));
            if (!pindex.has_value()) {
                return;
            }
        ...
        {
            // Find the last block the caller has in the main chain
            pindex = CBlockIndexOptRefConst(std::cref(*m_chainman.ActiveChainstate().FindForkInGlobalIndex(locator)));
            if (pindex.has_value())
