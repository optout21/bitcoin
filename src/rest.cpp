                         ...
                         const std::string& uri_part)
{
    ...
    ChainstateManager& chainman = *maybe_chainman;
    {
        ...
        auto pindex = std::optional<std::reference_wrapper<const CBlockIndex>>(*chainman.m_blockman.LookupBlockIndex(*hash));
        while (pindex.has_value() && active_chain.Contains(pindex->get())) {
            headers.push_back(&pindex->get());
            if (headers.size() == *parsed_count) {
                break;
            }
            pindex = active_chain.Next(pindex->get());
        }
    }
...
static bool rest_filter_header(const std::any& context, HTTPRequest* req, const std::string& uri_part)
{
    ...
    headers.reserve(*parsed_count);
    {
        ...
        LOCK(cs_main);
        CChain& active_chain = chainman.ActiveChain();
        auto pindex = std::optional<std::reference_wrapper<const CBlockIndex>>(*chainman.m_blockman.LookupBlockIndex(*block_hash));
        while (pindex.has_value() && active_chain.Contains(pindex->get())) {
            headers.push_back(&pindex->get());
            if (headers.size() == *parsed_count)
                break;
