                                       ...
                                       int64_t accept_time, bool bypass_limits, bool test_accept)
{
    ...
    /*state=*/active_chainstate.FlushStateToDisk(FlushStateMode::PERIODIC);
    return result;
}
...
BlockValidationState FatalError(Notifications& notifications, const bilingual_str& message)
{
    ...
    /*ret=*/state.Error(message.original);
    return state;
}
...
[[nodiscard]] static BlockValidationState CheckBlockHeader(const CBlockHeader& block, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits, consensusParams)) {
        /*ret=*/state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "high-hash", "proof of work failed");
        return state;
    }
    return state;
}
...
static BlockValidationState CheckWitnessMalleation(const CBlock& block, bool expect_witness_commitment)
{
    ...
    if (expect_witness_commitment) {
        if (block.m_checked_witness_commitment) return state;
 ...
 *  Note that -reindex-chainstate skips the validation that happens here!
 */
static BlockValidationState ContextualCheckBlock(const CBlock& block, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    for (const auto& tx : block.vtx) {
        if (!IsFinalTx(*tx, nHeight, nLockTimeCutoff)) {
            BlockValidationState state;
            /*ret=*/state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-txns-nonfinal", "non-final transaction");
            return state;
        }
    }
    ...
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_HEIGHTINCB))
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            BlockValidationState state;
            /*ret=*/state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-height", "block height mismatch in coinbase");
            return state;
        }
    }
    ...
    if (GetBlockWeight(block) > MAX_BLOCK_WEIGHT) {
        BlockValidationState state;
        /*ret=*/state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-weight", strprintf("%s : weight limit failed", __func__));
        return state;
    }
...
BlockValidationState ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        if (miSelf != m_blockman.m_block_index.end()) {
            ...
            if (pindex->nStatus & BLOCK_FAILED_VALID) {
                ...
                BlockValidationState state;
                /*ret=*/state.Invalid(BlockValidationResult::BLOCK_CACHED_INVALID, "duplicate-invalid",
    ...
    if (!min_pow_checked) {
        ...
        BlockValidationState state;
        /*ret=*/state.Invalid(BlockValidationResult::BLOCK_HEADER_LOW_WORK, "too-little-chainwork");
        return state;
    }
...
BlockValidationState ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    BlockValidationState state{AcceptBlockHeader(block, &pindex, min_pow_checked)};
    CheckBlockIndex();
